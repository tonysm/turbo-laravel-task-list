<x-turbo::frame id="create_task_list" x-data x-on:turbo:submit-end="$event.detail.success && $refs.form.reset()">
    <form x-ref="form" action="{{ route('task-lists.store') }}" method="post" class="w-full peer-has-[*]:mt-4">
        @csrf

        <div class="flex w-full items-center group rounded-lg border border-gray-300 dark:border-gray-700 overflow-hidden focus-within:ring-2 focus-within:outline-none focus-within:border-indigo-500 dark:focus-within:border-indigo-600 focus-within:ring-indigo-500 dark:focus-within:ring-indigo-600">
